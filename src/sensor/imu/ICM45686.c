...
int icm45_ext_passthrough(bool passthrough)
{
	...
	if (passthrough) {
		err |= ssi_reg_write_byte(
			...
			0x18
		); // AUX1_MODE_OVRD, AUX1 in I2CM Bypass
	} else {
		err |= ssi_reg_write_byte(
			SENSOR_INTERFACE_DEV_IMU,
			ICM45686_IOC_PAD_SCENARIO_AUX_OVRD,
...
int icm45_ext_setup()
{
	int err = 0;
	uint8_t ireg_buf[3];
	ireg_buf[0] = ICM45686_IPREG_BAR; // Bank Address

	// Config REG_60: AUX1_SCLK
	// Bit 6(PE)=1, Bit 5(PUD)=1 -> Enable Pull-up
	// Bit 4=1 -> I2C Master Mode Requirement (CRITICAL!)
	// Bit 3(PE)=1, Bit 2(PUD)=1 -> AUX1_CS Pull-up (Default)
	// Value: 0x7D (0111 1101)
	ireg_buf[1] = ICM45686_IPREG_BAR_REG_60;
	...
	// Config REG_61: AUX1_SDO / AUX1_SDI
	// Bits 4:3 (SDO) -> 11 (Pull-up Enable)
	// Bits 0:1 (SDI) -> 11 (Pull-up Enable)
	// Value: 0xBB (Reset value, 1011 1011) - explicitly setting it to be safe
	ireg_buf[1] = ICM45686_IPREG_BAR_REG_61;
	ireg_buf[2] = 0x80 | // Preserve default value of Bit 7 (INT1 PULL EN) to prevent affecting INT1
				  ICM45686_BIT_AUX1_SDO_PULL_UP | ICM45686_BIT_AUX1_SDO_PULL_EN | ICM45686_BIT_AUX1_SDI_PULL_UP
				| ICM45686_BIT_AUX1_SDI_PULL_EN;
	err |= ssi_burst_write(SENSOR_INTERFACE_DEV_IMU, ICM45686_IREG_ADDR_15_8, ireg_buf, 3);
	// AUX1_MODE_OVRD, AUX1 in I2CM Master, AUX1_ENABLE_OVRD, AUX1 enabled
...
int icm45_ext_write(const uint8_t addr, const uint8_t *buf, uint32_t num_bytes)
{
	...
	uint8_t dev_profile_data[2] = {0x00, addr};
	...
	do {
		...
		if (--timeout <= 0) {
			LOG_ERR("I2CM timeout");
			return -1;
		}
	} while (last_status & ICM45686_BIT_I2CM_STATUS_BUSY);

	if (last_status & (ICM45686_BIT_I2CM_STATUS_SDA_ERR | ICM45686_BIT_I2CM_STATUS_SCL_ERR)) {
		LOG_ERR("I2CM Hardware Bus Error! Status: 0x%02x (Check Pull-ups)", last_status);
		return -1;
	}
	...
	if (dev_status & 0x01) { // NACK received
		LOG_WRN("I2CM NACK on addr 0x%02x", addr);
		return -1;
	}
